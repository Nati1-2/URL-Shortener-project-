...
export default function DynamicShortLinkPage({
  ...
  const { getLinkByShortCode, recordClick } = useLinksStore();
  ...
  }, [shortCode, getLinkByShortCode, recordClick]);

  if (status === "loading" || status === "redirecting") {
    return (
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-6 text-center">
        <div className="bg-slate-900 border border-slate-800 p-8 rounded-2xl max-w-md w-full shadow-2xl flex flex-col items-center gap-4">
          ...
          <p className="text-slate-400 text-sm">
            Taking you to <span className="text-blue-400 font-mono break-all">{targetUrl || shortCode}</span>
          </p>
          {targetUrl && (
            <a
              href={targetUrl}
            ...
            >
              Click here if you are not redirected automatically <ExternalLink className="w-3 h-3" />
            </a>
          )}
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-6 text-center">
      <div className="bg-slate-900 border border-slate-800 p-8 rounded-2xl max-w-md w-full shadow-2xl flex flex-col items-center gap-4">
        ...
        <p className="text-slate-400 text-sm">
          The link <span className="font-mono text-slate-200">/{shortCode}</span> {status === "not_found" ? "does not exist or has been deleted." : "is currently inactive."}
        </p>
        <Link
          href="/dashboard"
