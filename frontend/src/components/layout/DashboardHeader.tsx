...
export const DashboardHeader: React.FC = () => {
  ...
  const [backendHealth, setBackendHealth] = useState<{ healthy: boolean; latencyMs: number } | null>(null);
  ...
  // Ping API Gateway on mount
  useEffect(() => {
    let isMounted = true;
    const checkGateway = async () => {
      const res = await apiClient.checkHealth();
      if (isMounted) setBackendHealth(res);
    };
    checkGateway();
  ...
  return (
    <>
      <header className="h-16 px-6 bg-white/85 dark:bg-[#080c14]/85 backdrop-blur-2xl border-b border-slate-200/80 dark:border-slate-800/80 flex items-center justify-between sticky top-0 z-30 transition-colors">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative max-w-md w-full hidden md:block">
            ...
            <input
              ...
              className="w-72 h-9 pl-10 pr-10 text-xs bg-slate-100 dark:bg-slate-900/90 border border-slate-200/80 dark:border-slate-800/80 text-slate-900 dark:text-slate-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/40"
            />
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5 ml-auto">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-bold border transition-all cursor-pointer bg-white dark:bg-slate-900 hover:border-blue-500/50 shadow-sm"
            title="Click to open Microservices Topology & Diagnostics"
          >
            <span
            ...
            <span className="text-slate-700 dark:text-slate-200">
              {backendHealth?.healthy ? `Gateway: ${backendHealth.latencyMs}ms` : "Hybrid Engine"}
            </span>
          ...
          <div className="relative" ref={userMenuRef}>
            <button
              ...
              className="flex items-center gap-2 pl-2 pr-1.5 py-1 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors cursor-pointer"
            >
              <img
                src={user?.avatar}
                alt={user?.name}
            ...
            {showUserMenu && (
              <div className="absolute right-0 mt-2 w-52 rounded-2xl glass-card bg-white/95 dark:bg-slate-900/95 border border-slate-200 dark:border-slate-800 p-2 shadow-2xl z-50 space-y-1 text-xs font-medium text-slate-700 dark:text-slate-300 animate-fadeIn">
                ...
                >
                  ...
                  <span className="text-[10px] text-emerald-500 font-bold">Diagnostics</span>
                </button>
